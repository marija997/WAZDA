...
function vratiPodatke() {
  fetch("http://localhost:3000/vratiSaStatCicibani")
    .then(response => {
      if (!response.ok)
        throw new Error(
...
//u tabeli su sample podaci cisto radi testiranja koda-- izmeniti kada se poveze sa bazom
function crtajTabelu(data) {
  const tabela = document.getElementById("cicibaniTabela");
  let tabelaHTML =
...
function pretraga() {
  ...
  var tabela = document.getElementById("cicibaniTabela");
